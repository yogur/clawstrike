...
- **gate tool response (post US-022):** Includes both `trust_level` (original channel-resolved) and `effective_trust_level` (after elevated-scrutiny downgrade). The decision matrix uses `effective_trust_level`. `elevated_scrutiny: bool` indicates whether the session was downgraded.
